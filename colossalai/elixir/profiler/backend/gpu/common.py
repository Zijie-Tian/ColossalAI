...
from elixir.device.exception import ZeusBaseGPUError
from elixir.logging import get_logger
from elixir.device.common import has_sys_admin
...
class GPU(abc.ABC):
    ...
    @abc.abstractmethod
    def getPowerManagementLimit(self) -> int:
...
class GPUs(abc.ABC):
    ...
    def getPowerManagementLimit(self, gpu_index: int) -> int:
        """Return the current power management limit. Units: mW."""
        return self.gpus[gpu_index].getPowerManagementLimit()
