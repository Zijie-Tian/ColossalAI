...
# End of Selection
from elixir.logging import get_logger
from elixir.device.utils.framework import sync_execution as sync_execution_fn
from elixir.device import get_gpus, get_cpus
from elixir.device.gpu.common import ZeusGPUInitError, EmptyGPUs
from elixir.device.cpu.common import ZeusCPUInitError, ZeusCPUNoPermissionError, EmptyCPUs
...
class Measurement:
    ...
    def summary(self) -> str:
        ...
        Returns:
            A formatted string with the total GPU energy and average CPU energy.
        """
        total_gpu_energy = sum(self.gpu_energy.values())
        avg_cpu_energy = (
            sum(self.cpu_energy.values()) / len(self.cpu_energy) if self.cpu_energy else None
        )
        summary_str = (
            f"Measurement Summary:\n"
            f"  Total GPU Energy: {total_gpu_energy:.2f} Joules\n"
            f"  Average CPU Energy: {avg_cpu_energy:.2f} Joules\n" if avg_cpu_energy is not None else
            f"  Average CPU Energy: Not available\n"
        )
        return summary_str

    def __repr__(self) -> str:
        gpu_energy_str = "\n    ".join(f"{k}: {v:.2f} Joules" for k, v in self.gpu_energy.items())
        cpu_energy_str = (
            "\n    ".join(f"{k}: {v:.2f} Joules" for k, v in self.cpu_energy.items())
            if self.cpu_energy else "Not available"
        )
        dram_energy_str = (
            "\n    ".join(f"{k}: {v:.2f} Joules" for k, v in self.dram_energy.items())
            if self.dram_energy else "Not available"
        )
        return (
            f"Measurement(\n"
            f"  time={self.time},\n"
...
def merge_measurements(measurements: list[Measurement]) -> Measurement:
    """Merge a list of Measurement instances into a single Measurement."""
    if not measurements:
        raise ValueError("The measurements list cannot be empty")

    merged = measurements[0]

    for measurement in measurements[1:]:
        merged.time += measurement.time

        for k, v in measurement.gpu_energy.items():
            merged.gpu_energy[k] = merged.gpu_energy.get(k, 0) + v

        if merged.cpu_energy is None:
            merged.cpu_energy = measurement.cpu_energy.copy() if measurement.cpu_energy else None
        elif measurement.cpu_energy is not None:
            for k, v in measurement.cpu_energy.items():
                merged.cpu_energy[k] = merged.cpu_energy.get(k, 0) + v

        if merged.dram_energy is None:
            merged.dram_energy = measurement.dram_energy.copy() if measurement.dram_energy else None
        elif measurement.dram_energy is not None:
            for k, v in measurement.dram_energy.items():
                merged.dram_energy[k] = merged.dram_energy.get(k, 0) + v

    # Calculate average CPU energy
    if merged.cpu_energy:
        for k in merged.cpu_energy:
            merged.cpu_energy[k] /= len(measurements)
