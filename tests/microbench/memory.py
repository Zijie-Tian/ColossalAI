...
import threading
import torch
# from artemis.exception import MemoryAllocationError
# from artemis.utils import artemis_logger
...
class MemoryAllocationError(Exception):
    ...
    def __init__(self, message="Not enough memory in buffer. Allocation failed"):
...
class ContinuousMemoryAllocator(object):

    def __init__(self, size, dtype, device):
        if device == 'cpu':
            self.buffer = torch.zeros(size, dtype=dtype, device=device).pin_memory()
        else :
        ...
        #! continuous memory size[0] is buffer size.
    ...
    def allocate_tensor(self, size, log=False):
        free_before = self.total_free

        # assert size <= self.total_free, "Not enough memory in buffer. Allocation failed"
        
        if log:
            print("Allocate tensor with size : {}, Total free : {}.".format(size, self.total_free))
        
        if size > self.total_free:
            raise MemoryAllocationError(message="Not enough memory in buffer. Allocation failed, {} > {}".format(size, self.total_free))
        ...
        if self.largest_contiguous < size:
            ...
            # print(self.id_to_params)
            
            #! this cause something wrong !!! no need to assign tensor to param.
        ...
        #! [Zijie Tian] Check if the tensor is purged.
        self.purged = False
    ...
    def assign_to_varible(self, tensor : torch.Tensor, varible : torch.Tensor, numel, shape):
        pass
    ...
    def release_tensor_with_id(self, tensor_id, log=False):
        free_before = self.total_free
        ...
        if log:
            print("Release tensor with id : {}, Total free : {}.".format(tensor_id, self.total_free))
    ...
    def print_allocation(self, resolution=200):
        s = self._get_allocation_str(resolution)
        print(s)
    
    def _get_allocation_str(self, resolution=200):
    ...
    def print_percentage(self):
        print("=" * 16, " Total size : {}, Usage : {} ".format(
            self.total_size / (1024 ** 3), 
            (self.total_size - self.total_free) / self.total_size
        ), "=" * 16)

    def max_allocated(self):
        return self.max_allocated
    
    def get_tensor_with_id(self, tensor_id):
        assert tensor_id in self.tensor_map.keys(), "Invalid tensor id {}.".format(tensor_id)
    ...
    def _get_new_tensor(self, address, size):
        ...
        new_tensor.artemis_tensor_id = tensor_id
    ...
    """
        _reset_memory reset all meta-data of the allocator, just like delete.
    """
    ...
    def copy_memory(self, src_mem, reassign_param=False):
        assert self.total_size >= (src_mem.total_size - src_mem.total_free), "Total Size too small."
        assert self.purged == True, "Memory not purged. Remember reset memory before copy."
        
        #> Must defragment.
        self._defragment_memory()
        src_mem._defragment_memory()
        
        # self.buffer.copy_(src_mem.buffer, non_blocking=True)
        #! TODO(zijie): Need modify this to online float32tohalf.
        self.buffer = src_mem.buffer.to(self.device, non_blocking=True)
        
        cpu_tensor_arr = list(src_mem.tensor_map.values())
        gpu_tensor_arr = _unflatten_dense_tensors(self.buffer, cpu_tensor_arr)
        
        #! TODO(zijie) : Need remember the map.
        #! Need benchmark.
        for idx, gpu_tensor in enumerate(gpu_tensor_arr):
            tensor_id = id(cpu_tensor_arr[idx])
            if reassign_param and tensor_id in src_mem.id_to_params.keys():
                for param in src_mem.id_to_params[tensor_id]:
                    self.tensor_map[id(gpu_tensor)] = gpu_tensor
                    self.assign_to_param(gpu_tensor, param, param.numel(), param.data.shape)
