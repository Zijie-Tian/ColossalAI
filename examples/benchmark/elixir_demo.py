...
from colossalai.elixir.utils import get_model_size, model_size_formatter
...
# from colossalai.elixir.profiler.energy_profiler import ZeusMonitor, Measurement

from common.models import get_model
from common.utils import fake_gpt_data, get_mem_info, get_profile_context, get_tflops, get_time_stamp
...
def main():
    ...
    # #> Print the Energy measurement result.
    # measurement = monitor.end_window("training")
    # logger.info(f"Measurement object: {measurement}", ranks=[0])
    # logger.info(f"Took {measurement.time} seconds.", ranks=[0])
    # for gpu_idx, gpu_energy in measurement.gpu_energy.items():
    #     logger.info(f"GPU {gpu_idx} consumed {gpu_energy} Joules.", ranks=[0])

    #> Print the TFLOPS result.
    tflops_list.sort()
