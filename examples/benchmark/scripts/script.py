...
from elixir.profiler.energy_profiler import ZeusMonitor, Measurement, merge_measurements
...
def main():
    ...
    elif args.dp_type == 'elixir_pp':
        raise NotImplementedError('Pipeline parallel is not supported yet')
    else:
        raise NotImplementedError
    ...
    #> Start monitoring.
    monitor = ZeusMonitor(gpu_indices=list(range(dist.get_world_size())), sync_execution_with="torch")
    ...
    measurements = [None for _ in range(dist.get_world_size())]
    dist.gather_object(measurement, measurements if dist.get_rank() == 0 else None, dst=0)

    if dist.get_rank() == 0:
        merged_measurement = merge_measurements(measurements)
        logger.info(f"Took {merged_measurement.time} seconds.")
        
        for gpu_idx, gpu_energy in merged_measurement.gpu_energy.items():
            logger.info(f"GPU {gpu_idx} consumed {gpu_energy} Joules.")
        for cpu_idx, cpu_energy in merged_measurement.cpu_energy.items():
            logger.info(f"CPU {cpu_idx} consumed {cpu_energy} Joules.")
